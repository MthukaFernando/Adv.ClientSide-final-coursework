...
function FavouritesPanel({ favourites, onRemove, onAdd, onRemoveAll }) {
  const [isOver, setIsOver] = useState(false);

  return (
    <div className="fav-panel-wrapper">
      <div
      ...
      >
        ...
        <div className="fav-header">
          ...
          {favourites.length > 0 && (
            <button className="remove-all-btn" onClick={onRemoveAll}>🗑️ Remove All</button>
          )}
        </div>
        {favourites.length === 0 ? (
      ...
      {favourites.length > 0 && (
        <button className="remove-all-btn-mobile" onClick={onRemoveAll}>🗑️ Remove All</button>
      )}
    </div>
  );
}
